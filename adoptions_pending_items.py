...
NS = {'ss': 'urn:schemas-microsoft-com:office:spreadsheet'} # define namespace for .xml file parsing
...
def load_file(root): # function to load an Excel (.xlsx or .xls) or CSV (.csv) file.
    ...
    while file_path is None:
        ...
        if not file_path:
            retry_dialog_result = messagebox.askretrycancel("No file selected", "Please select a file to proceed.") # messagebox to prompt user to select file if not selected, or cancel script
        ...
        try:
            if file_path.endswith('.xlsx'):
                ...
                try: # attempt to load .xls file
                    ...
                    conversion_attempt_box = messagebox.askquestion("Error", f"Failed to load. Attempt converting to .xlsx?") # prompt to convert .xls file to .xlsx if unsuccessful
                    if conversion_attempt_box == 'yes':
                        try:
                            ...
                            if not retry_dialog_result: # prompt user to try again, or cancel
                                break
                            file_path = None
                    else:
                        break
                ...
                retry_dialog_result = messagebox.askretrycancel("Error", "Unsupported file format. Please provide a valid Excel (.xlsx or .xls) or CSV (.csv) file.") # prompt user to re-select file
...
def rename_and_add_cols(df):
    ...
    alt_df = pd.merge(df1, df2, left_index=True, right_index=True) # combine dataframe 1 (original data) and dataframe 2 (new columns and data)
    print(alt_df)
    return alt_df
...
def fill_easy_cells(df): # function to fill cells with pre-existing data
    df['EXTERNAL_ID'] = df.apply(lambda row: f"{row['ISBN']}{'parent'}", axis=1) # fills 'EXTERNAL_ID' column with each row's 'ISBN' and string 'parent'
    print(f"{df['EXTERNAL_ID']}")
    df['ITEM_NAME_NUMBER'] = df.apply(lambda row: f"{row['DISPLAYNAME']}-{row['ISBN']}", axis=1) # fills 'ITEM_NAME_NUMBER' column with each row's 'DISPLAYNAME' and 'ISBN' with '-' between each
    print(f"{df['ITEM_NAME_NUMBER']}")
    df['Webstore Image Name'] = df.apply(lambda row: f"{row['ISBN']}", axis=1) # fills 'Webstore Image Name' field with each row's 'ISBN'
    print(f"{df['Webstore Image Name']}")

def label_dupes(row): # function to label duplicates as 'New' or 'Used' children items
    if row['occurrence'] == 1: # if first duplicate (not original), assign values as a 'New' item
...
def make_children(df): # function to create child items, making 2 extra copies of each row
    repeat_index = df.index.repeat(3)
    df_duplicated = df.loc[repeat_index].reset_index(drop=True) # creates new dataframe for original items, duplicate items, and resets index

    df_duplicated['original_index'] = repeat_index.to_numpy() # creates array([[External ID value 1, External ID value 2], [Item Name Number Value 1, Item Name Number Value 2]]) of duplicates from each 'original_index' item
    ...
    df_duplicated = df_duplicated.apply(label_dupes, axis=1) # use func to apply values in each row in given column based on conditional in label_dupes func
    print(df_duplicated)
...
def get_images(isbn): # function to call ISBNdb and retrieve book images
    ...
    try: # attempt to call ISBNdb and receive json response
        ...
        if e.response.status_code == 404: # API call responds with 404 error
            ...
            return
        elif e.response.status_code == 429: # API call responds with 429 error indicating 2000 call per day limit reached
...
def resize_covers(input_folder, output_folder, canvas_size): # function to resize book images to meet NS standards, filling white background if smaller than required dimensions
    ...
    for filename in os.listdir(input_folder):
        if filename.lower().endswith(('.jpg', '.jpeg')):
            ...
            try:
                img = Image.open(input_path).convert("RGB") # convert image to 'RGB' format
            except Exception as e:
                print(f"Error processing {filename}: \n{e}")
                return
            
            canvas_width, canvas_height = canvas_size # define width and height of canvas from passed-in 'canvas_size' object
            img_width, img_height = img.size # define width and height of image

            canvas = Image.new("RGB", canvas_size, (255, 255, 255)) # create new canvas in 'RGB' format at dimensions from 'canvas_size' object, and fill with white background

            if img_width > canvas_width or img_height > canvas_height:
                ratio_w = canvas_width / img_width # defines difference ratio if image is wider than canvas
                ratio_h = canvas_height / img_height # defines difference ratio if image is taller than canvas
                scale_factor = min(ratio_w, ratio_h) # determines the smaller of the two ratios to create scale factor

                new_width = int(img_width * scale_factor) # define new image width using the current width multiplied by 'scale_factor'
                new_height = int(img_height * scale_factor)# define new image height using the current height multiplied by 'scale_factor'
                resized_img = img.resize((new_width, new_height), Image.Resampling.LANCZOS) # create resized image with new width and height and adjust resolution as needed

                x_offset = (canvas_width - new_width) // 2 # define position for image on x-axis (width difference divided by 2)
                y_offset = (canvas_height - new_height) // 2 # define position for image on y-axis (height difference divided by 2)
                canvas.paste(resized_img, (x_offset, y_offset)) # paste resized image on canvas, using x and y offset values to determine center placement
...
def loading_popup(root, task_functions, title="Working", message="Please wait..."): # function to display "Loading" window while 'change_books' function runs
    ...
    worker_thread = threading.Thread(target=task_wrapper, daemon=True) # allows 'Loading' window to run on main thread and functions to run on different thread
    worker_thread.start()

def change_books(load_file, delete_junk_cols, reorder_cols, rename_and_add_cols, reorder_all_cols, fill_easy_cells, get_images, resize_covers, make_children): # function to run other functions
    try:
        df = load_file(root)
        if df is None:
            return
        ...
        with pd.ExcelWriter(excel_save_path, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name=excel_name, index=False) # save as Excel and ensure ISBN and UPC columns are 'number' format
